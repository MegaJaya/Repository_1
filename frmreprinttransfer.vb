...
Public Class frmreprinttransfer
    Dim wbsid_ As String = ""
    Private Sub frmreprinttransfer_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.Show()
    ...
    End Sub
    Sub tampil()
        da = New odbcDataAdapter("select * from wbs_product_transfer_tab where wbout<>0 and datein >= #" & Date.Parse(dtfrom.Text) & "# and datein <= #" & Date.Parse(dtto.Text) & "#  order by wbsid desc", Conn)
        ds = New DataSet
        ds.Clear()
        da.Fill(ds, "wbsdata")
    ...
    Private Sub btnreprint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnreprint.Click
        ...
        'MsgBox(prn)
        Dim a As Integer = 0
        a = CInt(IsiInteger("select cetak from wbs_product_transfer_tab where wbsid='" & wbsid_ & "'")) + 1
        rd.Close()
        cmd = New odbcCommand("update wbs_product_transfer_tab set [cetak]='" & Convert.ToString(a) & "'  where [wbsid]='" & wbsid_ & "'", Conn)
        cmd.ExecuteNonQuery()
        dgreprint.Refresh()


        cryRpt.Load(Application.StartupPath & "\report\PrintWPTransfer.rpt")
        cryRpt.SetDatabaseLogon("Admin", "w@b@s4ms4system2all")
        cr2.Visible = True
        cr2.ShowCloseButton = True
        cr2.SelectionFormula = "{wbs_product_transfer_tab.wbsid}='" & wbsid_ & "'"
