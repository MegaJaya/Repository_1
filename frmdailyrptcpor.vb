...
Public Class frmdailyrptcpor
    ...
    Sub printrpt()
        ...
        cryRpt.Load(Application.StartupPath & "\report\dailyreport_CPOT_receive .rpt")
        cryRpt.SetDatabaseLogon("Admin", "w@b@s4ms4system2all")
        crdr.Visible = True
        crdr.ShowCloseButton = True
        crdr.SelectionFormula = "{wbs_product_receive_tab.dateout} in date (" & dfrom & ") to date (" & dto & ") and {wbs_product_receive_tab.wbout} <> 0 and {wbs_product_receive_tab.InventoryPart}='" & cbpart.SelectedItem & "'"
